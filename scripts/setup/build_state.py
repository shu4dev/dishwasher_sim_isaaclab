...
planned against until its geometry has been extracted and decomposed — per object class. That is
two scripts times N classes, one of which needs Kit, which is exactly the sort of chain that
gets typed wrong at 5 pm.

This runs the chain in the right order and stops at the first real failure.
...
def stages(obj: str) -> list:
    """The two-stage chain for one (state, class), in dependency order."""
