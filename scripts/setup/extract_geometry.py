...
Builds the locked statics scene, settles it, then writes per-rigid-body meshes (statics +
the active object's body-frame OBJ) + ``scene_state.json`` to ``assets/cache/``. Runs with
...
parser = argparse.ArgumentParser(description="Dump the collision-world cache from the settled scene.")
...
# scenario BEFORE scene/machine imports — they bind rack targets + the derived USD at import
if args_cli.machine:
