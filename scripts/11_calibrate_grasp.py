...
"""Phase C2: calibrate the contact pinch grasp on the welded mug.

With the mug welded at a candidate carry pose (rim plane ``--rim_z`` from the pad map printed
by ``scripts/10_v0_scene.py --measure``), this script:
...
Run with:
    scripts/run_kit.sh scripts/11_calibrate_grasp.py --headless --enable_cameras \
...
def main() -> None:
    ...
    with open(doc, "w") as f:
        f.write("# Grasp calibration (generated by scripts/11_calibrate_grasp.py)\n\n")
