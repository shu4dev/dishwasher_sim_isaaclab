...
All the environments are registered in the `dishwasher_tasks` extension. They start
...
with contextlib.suppress(ImportError):
    import dishwasher_tasks.tasks_experimental  # noqa: F401
...
def main():
    """Print all environments registered in `dishwasher_tasks` extension."""
    # Collect matching task specs first so we can enumerate presets in one pass.
    task_specs = [
        spec
        for spec in gym.registry.values()
        if "Dishwasher" in spec.id and (args_cli.keyword is None or args_cli.keyword in spec.id)
    ]
