...
"""Every tunable of the arrangement pipeline, in one place.

Values marked *measured* come from ``docs/joint_report.md`` (generated by the retired
inspect_scene.py, git history); do not eyeball-edit them. Frame convention everywhere:
**base frame, meters, Z-up, quaternions XYZW** (world frame = base frame shifted by
``ROBOT_BASE_POS_W`` — a robot-era mount kept as the FROZEN CACHE ANCHOR; every cached
coordinate is expressed in it, and it feeds ``geometry.config_hash``).
...
# ---------------------------------------------------------------------------------------------
# FROZEN CACHE ANCHOR — the base frame every cached coordinate is expressed in (a robot-era
# mount; feeds geometry.config_hash). Never tune: a change invalidates every shipped cache.
ROBOT_BASE_POS_W = (0.0, 0.0, 0.25)
ROBOT_BASE_QUAT_W = (0.0, 0.0, 0.0, 1.0)
PEDESTAL_SIZE = (0.25, 0.25, 0.25)
PEDESTAL_POS_W = (0.0, 0.0, 0.125)
# dishwasher articulation-root (E_body_5) spawn pose: front face toward the base anchor,
# handle centered at x ~ 0.64 when the door is closed (measured RL placement, joint_report.md)
DISHWASHER_POS_W = (0.6498, 0.2423, 0.0)
DISHWASHER_QUAT_W = (0.0, 0.0, -0.70710678, 0.70710678)  # yaw -90 deg

# FROZEN CACHE ANCHOR — robot-era home pose; feeds geometry.config_hash only. Never tune.
HOME_Q = (0.0, -1.712, 1.712, -1.571, -1.571, 0.0)
...
# Rack-state scenarios — the two real INITIAL states of a stowed/open machine. Each carries a
# rack_action (a drive-target ramp on the named prismatic joint) that brings BOTH scenarios to
# the same placement state (lower rack out, upper rack in — the geometry the placement
# pipeline is validated on). "both_out" is the baseline (legacy assets/cache/, media/<phase>).
SCENARIOS = {
...
def apply_scenario(name: str) -> None:
    ...
    Must run before importing :mod:`dishsim.machine`/:mod:`dishsim.scene` (they bind rack
...
# ---------------------------------------------------------------------------------------------
# FROZEN CACHE ANCHORS — robot-era grasp constants that feed geometry.config_hash (per class,
# via set_active_object/grasp_transform). Nothing else reads them. Never tune: a change
# invalidates every shipped cache in the public archive.
# ---------------------------------------------------------------------------------------------
# Grasp aperture, measured 2026-08-10 by the retired calibrate_grasp.py (theta_touch + delta on
# the 0.85-scaled public YCB scan mug).
GRIPPER_APERTURE_GRASP_RAD: float | None = 0.106
# wrist_3_link -> TCP, measured 2026-07-29 by the retired check_scene.py --measure.
T_WRIST3_TCP_POS = (0.0, 0.130, 0.0)
T_WRIST3_TCP_QUAT: tuple[float, float, float, float] | None = (-0.5, -0.5, -0.5, 0.5)

# ---------------------------------------------------------------------------------------------
# active object view (rewritten in place by set_active_object)
# ---------------------------------------------------------------------------------------------
...
# values come from grasp_transform(spec) inside set_active_object. Feeds geometry.config_hash
# only ("grasp" key). Derivation (z-up scan frame at scale 0.85, 2026-08-10):
...
TASK: dict = {
    ...
    # derivation order; a tuple restricts and orders the pool (front-to-back, say).
    "slot_pools": {
        "floor_stand": None,
        "plate_slot": None,
        "basket_drop": None,
    ...
    # Objects of a type consume the list in order; a slot already taken, overlapping an
    # assigned one, or not placeable for that class is SKIPPED rather than failing. Falls back
    # to slot_pools[mode], then to every derived slot, when a type has no entry here.
    #
...
class GraspSpec:
    """FROZEN CACHE ANCHOR — robot-era grasp parameters, kept ONLY because they feed
    geometry.config_hash per class (grasp_transform() -> GRASP_TCP_OBJ_*, and
    aperture_rad/grasp_width_m -> GRIPPER_APERTURE_GRASP_RAD, both inside
    set_active_object). Never tune: a change invalidates that class's shipped caches.

    Families (carry orientation + contact patch): ``rim_diam`` (mug family), ``rim_edge``
    (bowl, container), ``edge_pinch`` (flat discs), ``handle_pinch`` (cutlery/utensils),
    ``stem_pinch`` (registry-only).
    """
    ...
    aperture_rad: float | None = None  # calibrated (robot era); None -> linear-jaw estimate
    force_min_n: float | None = None  # calibrated bands (robot era; unused, part of the record)
    force_max_n: float | None = None
    force_exec_max_n: float | None = None
...
def active_object(name: str):
    ...
    Only valid after :mod:`dishsim.scene`/:mod:`dishsim.machine` are imported — those bind the
...
def set_active_object(name: str) -> None:
    ...
    Must run before importing :mod:`dishsim.machine`/:mod:`dishsim.scene` (they bind the
    ...
    # every shipped cache hash stays byte-stable
...
# BEFORE importing dishsim.machine/dishsim.scene; geometry.config_hash reads these attributes
...
#: the rail-limited middle/third racks). The authored USD uses these exact per-joint limits.
...
# FROZEN CACHE ANCHORS — named base-frame anchors per machine (robot-era mount candidates).
# Every cached coordinate is expressed in the active anchor's frame and non-default anchors
# key geometry.config_hash, so the Bosch caches (baked at "side_winner", the A2-study
# winner) are only addressable through apply_base_placement("side_winner"). "front" is the
# frozen v1 pose — its literals must stay byte-identical to the module-top values.
# Quaternions are XYZW yaw-about-z. Pedestal is the support column (scene furniture).
...
#: (measured on the Bosch counter at the side_winner anchor, 1 cm inset).
...
def apply_base_placement(name: str) -> None:
    """Activate a named base-frame anchor for the ACTIVE machine (frozen cache anchors).
